 ...
 * gcc -levent placeholder.c servercheck.c valuelist.c daemon.c -o placeholder
 */
...
int main(int argc, char **argv) {
    ...
    if (do_daemonize) {
        ...
        res = daemonize(0, 0);
        if (res == -1) {
            printf("failed to daemon() in order to daemonize\n");
            exit(-1);
        }
        ...
        if (fd < 0) {
            printf("Can't open logfile %s for writing\n", log_file);
            exit(-1);
        }        
